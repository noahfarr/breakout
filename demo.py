...
def make_breakout(num_envs=1, render_mode="rgb_array"):
    from breakout.puffer_breakout import PufferBreakout

    return PufferBreakout(render_mode=render_mode)
...
def sweep_carbs(args, env_name, make_env, policy_cls, rnn_cls):
    ...
    if "env" in sweep_parameters:
        ...
        if "reward_tower" in env_params:
            param_spaces.append(
                carbs_param(
                    "env", "reward_tower", "linear", sweep_parameters, search_center=4.0
                )
            )
    ...
    param_spaces += [
        ...
        carbs_param(
            "train", "learning_rate", "log", sweep_parameters, search_center=1e-3
        ),
        carbs_param("train", "gamma", "logit", sweep_parameters, search_center=0.95),
        carbs_param(
            "train", "gae_lambda", "logit", sweep_parameters, search_center=0.90
        ),
        carbs_param(
            "train",
            "update_epochs",
        ...
        carbs_param(
            "train", "vf_clip_coef", "logit", sweep_parameters, search_center=0.1
        ),
        carbs_param(
            "train", "max_grad_norm", "linear", sweep_parameters, search_center=0.5
        ),
    ...
    def main():
        ...
        env_suggestion = {
            k.split("/")[1]: v for k, v in suggestion.items() if k.startswith("env/")
        }
        ...
        try:
            stats, uptime, new_elos = train(
                args, make_env, policy_cls, rnn_cls, wandb, elos=elos
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb-entity", type=str, default="noahfarr")
    ...
    if args["mode"] == "train":
        ...
        cProfile.run(
            "train(args, make_env, policy_cls, rnn_cls, wandb=None)", "stats.profile"
        )
        import pstats
